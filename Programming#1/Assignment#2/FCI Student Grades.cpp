...
string **gpa(double **arr_2 ,int x , int y){
	...
	for(int i=0;i<x;i++){
		for(int j=0;j<y;j++){
			...
			else if(arr_2[i][j]==85 || arr_2[i][j]<90){
				arr_3[i][j]="A";
			}
			else if(arr_2[i][j]==90 || arr_2[i][j]<=100){
				arr_3[i][j]="A+";
			}
			else{
						arr_3[i][j]="Error";
...
int main(){
	...
	for(int i=0;i<st;i++){
		...
		if(AVST[i]<50){
			...
			else if(AVST[i]==85 || AVST[i]<90){
				cout<<"A\n";
			}
			else if(AVST[i]==90 || AVST[i]<=100){
				cout<<"A+\n";
			}
			else{
					cout<<"Error";
	...
	//	 cout<<AVSU[i]<<" D+"<<endl;
		...
		if(AVSU[i]<50){
			...
			else if(AVSU[i]==85 || AVST[i]<90){
				cout<<"A\n";
			}
			else if(AVSU[i]==90 || AVST[i]<=100){
				cout<<"A+\n";
			}
			else{
					cout<<"Error";
